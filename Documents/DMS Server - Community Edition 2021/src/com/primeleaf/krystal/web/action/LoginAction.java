...
import com.primeleaf.krystal.web.controller.ConnectionManagerController;
...
public class LoginAction implements Action {
	...
	public WebView execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(loggedInUser == null){
			...
			if(loginStatus==3 && ConfigParser.multiLogin()) {
				loginStatus = 0;
			}
			switch (loginStatus){
			...
			case LoginModule.LOGIN_SUCCESSFUL:
				...
				session.setMaxInactiveInterval(ConfigParser.InactiveTimeout());
				User user = UserDAO.getInstance().readUser(loginId);
				user.setIpAddress(request.getRemoteAddr());
				session.setAttribute(HTTPConstants.SESSION_KRYSTAL,user);
				
				ConnectionManagerController CMC = new ConnectionManagerController();
				session.setAttribute(loginId,CMC);
			...
			Connection connection = ConnectionPoolManager.getInstance().getConnection();
			Statement stmt = connection.createStatement();
			StringBuffer hitListQuery =  new StringBuffer("Update USERS set LOGGEDIN='Y' Where USERID="+loggedInUser.getUserId());
			stmt.execute(hitListQuery.toString());
			connection.close();
