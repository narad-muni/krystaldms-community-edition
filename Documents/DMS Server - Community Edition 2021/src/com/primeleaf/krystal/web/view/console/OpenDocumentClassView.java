...
public class OpenDocumentClassView extends WebView {
	...
	private void printHitlist() throws Exception{
		...
		if(documentHitlist.size() > 0 ){
			...
			out.println("<th></th>");
			out.println("<th class=\"text-center\"><i class=\"bi bi-file-earmark text-primary\"></i></th>");
			...
			for(Hit hit : documentHitlist){
				...
				out.println("<img src=\"/images/"+StringHelper.getIconFileNameForExtension(hit.extension.toUpperCase())+ ".gif\" class=\"img-icon tip\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\" title=\"View Document : "+hit.extension.toUpperCase()+"\">");
				out.println("</a>");
				out.println("</td>");
