		...
		try {
			if(documentClass.getIndexDefinitions().size() > 0 ){
				...
				for(IndexDefinition indexDefinition : documentClass.getIndexDefinitions()){
					...
					out.println("<td class=\"text-center\">");
					out.println("<a href=\"/cpanel/deleteclassindex?name="+indexDefinition.getIndexColumnName()+"&classid="+documentClass.getClassId()+"\" class=\"confirm btn btn-xs btn-danger\" title=\"Are you sure, you want to delete this Index from the Document Class?\"><i class=\"bi bi-trash\"></i></a>");
					out.println("</td>");
					out.println("</tr>");
				}// for
